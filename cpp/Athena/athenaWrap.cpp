...
void AthenaWrapper::SetTempFolder(std::wstring folderPath)
{
...
bool AthenaWrapper::AthenaSendFile(void)
{
	...
	// upload file
	auto handle = std::async(std::launch::async, [] (AthenaOptionsPtr pOptions, const wchar_t* pfullpath)->bool 
	{
		std::wstring tmpFullPath(pfullpath);

		AthenaStatus aStatus = athenaUpload(pOptions, tmpFullPath.c_str(), L"json");
		if (aStatus != AthenaStatus::kSuccess)
		{
			return false;
		}
		...
		// clear temp file
		int res = std::remove(ws2s(tmpFullPath).c_str());
		if (res != 0)
			return false;

		return true;
	}, 
	m_athenaOptions, fullPath.c_str());
