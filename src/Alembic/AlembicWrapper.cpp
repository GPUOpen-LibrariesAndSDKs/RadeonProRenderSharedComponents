...
namespace RPRAlembicWrapper
{
	...
	inline Matrix4x4f to(M44d m) 
	{
		...
		for (int i = 0; i < 16; ++i) 
		{
			r.m_value[i] = (float)m.getValue()[i];
		}
	...
	inline bool parse_attributes(
		ICompoundProperty parent, 
		const std::string &key, 
		ISampleSelector selector, 
		std::shared_ptr<AttributeColumn> &attributeColumn, 
		std::string &geoScope
	) 
	{
		...
		else if (IFloatArrayProperty::matches(*header, kNoMatching)) 
		{
			...
			int component_size = extent * arrayExtent;
			int size = 0;
	...
	static auto parse_attributes(
		...
		ICompoundProperty compound_prop, 
		ISampleSelector selector
	) 
	{
		...
		for (int i = 0; i < compound_prop.getNumProperties(); ++i) 
		{
			...
			if (parse_attributes(compound_prop, key, selector, attributes, geoScope)) 
			{
				...
				{
					vertices->m_sheet.emplace_back(key, attributes);
				}
		...
		if (points) 
		{
			std::sort(points->m_sheet.begin(), points->m_sheet.end());
		}

		if (vertices) 
		{
			std::sort(vertices->m_sheet.begin(), vertices->m_sheet.end());
		}
	...
	inline void parse_polymesh(
		IPolyMesh polyMesh, 
		std::shared_ptr<PolygonMeshObject> polymeshObject, 
		ISampleSelector selector
	) 
	{
	...
	inline void parse_curves(
		ICurves curves, 
		std::shared_ptr<CurveObject> curveObject, 
		ISampleSelector selector
	) 
	{
	...
	static void parse_common_property(
		IObject o, 
		SceneObject *object, 
		const std::vector<M44d> &xforms, 
		ISampleSelector selector
	) 
	{
	...
	bool AlembicStorage::open(const std::string &filePath, std::string &error_message) 
	{
		try 
		{
			m_alembicArchive = std::shared_ptr<void>();
			m_alembicArchive = std::shared_ptr<void>(new IArchive(Alembic::AbcCoreOgawa::ReadArchive(), filePath), [](void *p) 
			{
				IArchive *archive = static_cast<IArchive *>(p);
				delete archive;
			});

			auto top = top_of_archive(m_alembicArchive);
	...
	std::shared_ptr<AlembicScene> AlembicStorage::read(uint32_t index, std::string &error_message) const 
	{
		...
		try 
		{
			...
			parse_object(top_of_archive(m_alembicArchive), selector, scene->objects);
			return scene;
		}
		catch (std::exception &e) 
		{
